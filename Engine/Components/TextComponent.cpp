...
namespace digi
{
	...
	{
		renderer.Draw(m_texture, m_owner->GetTransform(), registration);
	}

	void TextComponent::SetText(const std::string& text)
	{
		// create a texture from a surface generated in the font class 
		m_texture->CreateFromSurface(m_font -> CreateSurface(text, color), g_Ren);
	}
	...
	bool TextComponent::Read(const rapidjson::Value& value)
	{
		...
		m_font = g_Resource.Get<Font>(font_name, font_size);
